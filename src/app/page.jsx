    ...
    <>
      <section className="hero" aria-labelledby="hero-title">
        <div className="hero-slider" aria-hidden="true">
          {heroSlides.map((slide, index) => {
            const slideSrc = slide.image?.src || slide.image;
            return (
              <div
                key={slide.alt}
                className={`hero-slide ${index === currentSlide ? "active" : ""}`}
              >
                <img src={slideSrc} alt={slide.alt} />
              </div>
            );
          })}
        </div>
      ...
      <section className="hero-support" aria-labelledby="hero-support-title">
        ...
        <div className="hero-support-cards">
          {heroSupportCards.map(({ title, description, to, Icon }) => (
            <Link className="hero-support-card" href={to} key={title}>
      ...
      >
        <img
          ...
          src={getInvolvedImage.src || getInvolvedImage}
          ...
          <div className="home-gallery-track track-left">
            <div className="home-gallery-track-inner">
              {[...galleryRowOne, ...galleryRowOne].map((item, idx) => {
                const itemSrc = item.image?.src || item.image;
                return (
                  <div className="home-gallery-card" key={`r1-${idx}`}>
                    <img src={itemSrc} alt={item.imageAlt} loading="lazy" />
                  </div>
                );
              })}
            </div>
